defmodule Streampai.Repo.Migrations.AddViewerTables do
  ...
  def change do
    ...
    create unique_index(:viewer_identities, [:platform, :platform_user_id],
             name: :idx_viewer_identities_platform_user
           )

    create index(:viewer_identities, [:viewer_id, :platform],
             name: :idx_viewer_identities_viewer_platform
           )
    ...
    create constraint(:viewer_identities, :valid_linking_method,
             check:
               "linking_method IN ('automatic', 'manual', 'username_similarity', 'cross_platform_activity', 'admin_override')"
           )
    ...
    create index(:viewer_linking_audits, [:viewer_identity_id],
             name: :idx_viewer_linking_audits_identity_id
           )

    create index(:viewer_linking_audits, [:linking_batch_id],
             name: :idx_viewer_linking_audits_batch_id
           )

    create index(:viewer_linking_audits, [:action_type],
             name: :idx_viewer_linking_audits_action_type
           )

    create index(:viewer_linking_audits, [:created_at],
             name: :idx_viewer_linking_audits_created_at
           )

    create index(:viewer_linking_audits, [:algorithm_version],
             name: :idx_viewer_linking_audits_algorithm_version
           )

    # Add constraint for action_type enum
    create constraint(:viewer_linking_audits, :valid_action_type,
             check:
               "action_type IN ('create', 'update', 'unlink', 'relink', 'confidence_update', 'username_update')"
           )
    ...
    create index(:chat_messages, [:viewer_id, :inserted_at],
             name: :idx_chat_messages_viewer_chrono
           )
