...
format:
    ...
    cd cdk; bun format; bun lint -- --write --unsafe
...
si:
    ...
    echo ""
    iex --name streampai@127.0.0.1 --cookie streampai -S mix phx.server
...
dev:
    ...
    echo "   Phoenix:  http://localhost:$PHOENIX_PORT (interactive)"
    ...
    echo "   You have an interactive Elixir shell (iex) in this terminal"
    echo ""
    ...
    echo "⏳ Waiting for PostgreSQL..."
    for i in {1..30}; do
    	if PGPASSWORD=postgres psql -U postgres -h localhost -p "${DB_PORT:-5432}" -c "SELECT 1" >/dev/null 2>&1; then
    		break
    	fi
    	sleep 1
    done
    ...
    # Start Phoenix in foreground with interactive shell (named node for Livebook)
    iex --name streampai@127.0.0.1 --cookie streampai -S mix phx.server
...
livebook:
    #!/usr/bin/env bash
    echo "Starting Livebook attached to streampai node..."
    echo "Make sure 'just dev' or 'just si' is running first."
    echo ""
    RELEASE_NODE=livebook@127.0.0.1 RELEASE_COOKIE=streampai livebook server
...
worktree-setup:
    ...
    # ('just dev' will start them when you run it)
    ...
    # Stop containers - 'just dev' will start them
    echo ""
...
# Kill processes running on dev ports (Phoenix, Frontend, Caddy)
kill-ports:
    #!/usr/bin/env bash
    set -a
    source <(grep -v '^#' .env | grep -v '^$') 2>/dev/null || true
