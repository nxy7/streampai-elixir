defmodule StreampaiTest.LoadTestFramework do
  ...
  def simulate_streaming_session(num_users \\ 5, session_duration_ms \\ 60_000) do
    ...
    try do
      ...
      user_tasks = for i <- 1..num_users do
        user_id = "streamer_#{i}"
        Task.async(fn -> simulate_single_streamer(test_config, user_id, session_duration_ms) end)
      end
  ...
  defp generate_continuous_events_loop(config, start_time) do
    ...
    if current_time - start_time < config.duration_ms do
      ...
      case :rand.uniform(100) do
        n when n <= 5 ->  # 5% chance - donation
          generate_donation_event(user_id)
        n when n <= 25 -> # 20% chance - follow  
          generate_follow_event(user_id)
        n when n <= 30 -> # 5% chance - subscription
          generate_subscription_event(user_id)
        n when n <= 31 -> # 1% chance - raid
          generate_raid_event(user_id)
        _ ->
          :ok # No event this cycle
      end
  ...
  defp run_test_scenario(config) do
    ...
    user_tasks = for i <- 1..config.users do
      user_id = "load_test_user_#{i}"
      Task.async(fn -> simulate_user_activity(config.test_config, user_id, config.duration_ms) end)
    end
      ...
      case :rand.uniform(10) do
        n when n <= 3 ->
          ...
          simulate_user_loop(test_config, user_id, start_time, duration_ms, 
                           %{metrics | chat_messages_sent: metrics.chat_messages_sent + 1})

        n when n <= 5 ->
          # Update stream metadata (20% chance) 
          update_test_stream_metadata(user_id)
          simulate_user_loop(test_config, user_id, start_time, duration_ms,
                           %{metrics | metadata_updates: metrics.metadata_updates + 1})
  ...
  defp start_streaming_simulation(test_config, user_id, duration_ms, metrics) do
    ...
    event_generation_task = Task.async(fn -> 
  ...
  defp generate_events_loop(user_id, start_time, duration_ms) do
    ...
    if current_time - start_time < duration_ms do
      ...
      case :rand.uniform(1000) do
        n when n <= 2 ->  # 0.2% chance - donation
          generate_donation_event(user_id)
        n when n <= 20 -> # 1.8% chance - follow  
          generate_follow_event(user_id)
        n when n <= 25 -> # 0.5% chance - subscription
          generate_subscription_event(user_id)
        n when n <= 26 -> # 0.1% chance - raid
          generate_raid_event(user_id)
        _ ->
          :ok # No event this cycle
      end
  ...
  defp compile_load_test_results(config, user_results, event_results, monitoring_results) do
    total_events = Enum.reduce(user_results, 0, fn
    ...
    %{
      ...
      results: %{
        ...
        peak_memory_mb: monitoring_results |> Enum.map(& &1.memory_usage) |> Enum.max() |> div(1024 * 1024),
