defmodule Streampai.LivestreamManager.CloudflareAPIClient do
  @moduledoc """
  ...
  def init(:ok) do
    ...
    state = %__MODULE__{
      ...
      rate_limit_remaining: 1200,  # Cloudflare default
  ...
  """
  def create_live_input(name, opts \\ %{}) do
    GenServer.call(__MODULE__, {:create_live_input, name, opts}, 10_000)
  end

  @doc """
  Deletes a live input.
  """
  def delete_live_input(input_id) do
    GenServer.call(__MODULE__, {:delete_live_input, input_id}, 10_000)
  end

  @doc """
  Creates a live output for a platform.
  """
  def create_live_output(input_id, output_config) do
    GenServer.call(__MODULE__, {:create_live_output, input_id, output_config}, 10_000)
  end

  @doc """
  Updates live output configuration.
  """
  def update_live_output(output_id, config) do
    GenServer.call(__MODULE__, {:update_live_output, output_id, config}, 10_000)
  end

  @doc """
  Enables/disables a live output.
  """
  def toggle_live_output(output_id, enabled) do
    GenServer.call(__MODULE__, {:toggle_live_output, output_id, enabled}, 10_000)
  end
  ...
  def handle_call({:create_live_input, name, opts}, _from, state) do
    ...
    case make_api_request(state, :post, "/accounts/#{state.account_id}/stream/live_inputs", payload) do
  ...
  def handle_call({:delete_live_input, input_id}, _from, state) do
    case make_api_request(state, :delete, "/accounts/#{state.account_id}/stream/live_inputs/#{input_id}") do
  ...
  defp mock_api_response(state, :post, path, payload) do
    cond do
      String.contains?(path, "live_inputs") and not String.contains?(path, "outputs") ->
        ...
        response = %{
          "result" => %{
            "uid" => "live_input_" <> (:crypto.strong_rand_bytes(8) |> Base.encode16(case: :lower)),
            "rtmps" => %{
  ...
  defp update_rate_limits(state) do
    %{state | 
      ...
      rate_limit_reset: if(state.rate_limit_remaining <= 1, do: System.system_time(:second) + 300, else: state.rate_limit_reset)
    }
  end
