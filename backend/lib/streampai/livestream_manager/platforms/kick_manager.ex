defmodule Streampai.LivestreamManager.Platforms.KickManager do
  ...
  def init({user_id, config}) do
    ...
    Logger.info("[KickManager:#{user_id}] Started - #{DateTime.utc_now()}")
    {:ok, state}
  end
  ...
  def handle_info(:log_activity, state) do
    if state.is_active do
      Logger.info("[KickManager:#{state.user_id}] Streaming active - #{DateTime.utc_now()}")
    else
      Logger.debug("[KickManager:#{state.user_id}] Standby - #{DateTime.utc_now()}")
    end
  ...
  def handle_info(msg, state) do
    Logger.debug("[KickManager:#{state.user_id}] Unknown message: #{inspect(msg)}")
    {:noreply, state}
  end

  @impl true
  def handle_call({:start_streaming, stream_uuid}, _from, state) do
    Logger.info("[KickManager:#{state.user_id}] Starting stream: #{stream_uuid}")
  ...
  def handle_call(:stop_streaming, _from, state) do
    Logger.info("[KickManager:#{state.user_id}] Stopping stream")
  ...
  def handle_call({:send_chat_message, message}, _from, state) do
    Logger.info("[KickManager:#{state.user_id}] Sending chat message: #{message}")
    {:reply, :ok, state}
  end

  @impl true
  def handle_call({:update_stream_metadata, metadata}, _from, state) do
    Logger.info("[KickManager:#{state.user_id}] Updating metadata: #{inspect(metadata)}")
    {:reply, :ok, state}
  end

  @impl true
  def handle_call(request, _from, state) do
    Logger.debug("[KickManager:#{state.user_id}] Unhandled call: #{inspect(request)}")
    {:reply, :ok, state}
  end

  @impl true
  def handle_cast(request, state) do
    Logger.debug("[KickManager:#{state.user_id}] Unhandled cast: #{inspect(request)}")
    {:noreply, state}
  end
