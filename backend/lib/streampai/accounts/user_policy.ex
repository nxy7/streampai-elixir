defmodule Streampai.Accounts.UserPolicy do
  ...
  def can_impersonate_user?(user, target_user) do
    case {user_role(user), user_role(target_user)} do
      {:admin, :regular} -> true
      {:admin, :admin} -> false  # Admins cannot impersonate other admins
      {:regular, _} -> false     # Regular users cannot impersonate anyone
    end
  end
  ...
  def can_access?(user, resource) do
    case resource do
      ...
      :streaming_account_management -> true  # All users can manage their own accounts
