        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-sm text-gray-500">
                <%= @usage.hours_used %>/<%= if @usage.hours_limit == :unlimited, do: "∞", else: "#{@usage.hours_limit}h" %>
              </span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-2">
              <%= if @usage.hours_limit == :unlimited do %>
                <div class="bg-green-500 h-2 rounded-full transition-all duration-300" style="width: 25%"></div>
              <% else %>
                <div class={"bg-purple-500 h-2 rounded-full transition-all duration-300"} style={"width: #{min((@usage.hours_used / @usage.hours_limit) * 100, 100)}%"}></div>
              <% end %>
            </div>
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-sm text-gray-500">
                <%= @usage.platforms_used %>/<%= if @usage.platforms_limit == :unlimited, do: "∞", else: @usage.platforms_limit %>
              </span>
            </div>
            ...
            <p class="text-xs text-gray-500 mt-1">
              ...
              <% else %>
                <%= if @usage.platforms_used >= @usage.platforms_limit, do: "Platform limit reached", else: "#{@usage.platforms_limit - @usage.platforms_used} more platform(s) available" %>
              <% end %>
            </p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class={"border-2 rounded-lg p-4 transition-all duration-200 #{if @current_plan == "free", do: "border-purple-500 bg-purple-50", else: "border-gray-200 hover:border-gray-300"}"}>
            ...
            <%= if @current_plan == "free" do %>
              <button disabled class="w-full bg-gray-100 text-gray-400 py-2 px-4 rounded-lg text-sm cursor-not-allowed">
                Current Plan
              </button>
            <% else %>
              <button phx-click="downgrade_to_free" class="w-full bg-gray-600 text-white py-2 px-4 rounded-lg text-sm hover:bg-gray-700 transition-colors">
                Downgrade to Free
              </button>
            <% end %>
          </div>
          ...
          <div class={"border-2 rounded-lg p-4 transition-all duration-200 #{if @current_plan == "pro", do: "border-purple-500 bg-purple-50", else: "border-gray-200 hover:border-gray-300"}"}>
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="text-lg font-bold text-gray-900"><%= Streampai.Pricing.monthly_price_formatted() %><span class="text-sm font-normal text-gray-500">/mo</span></span>
            </div>
            <ul class="text-sm text-gray-600 space-y-1 mb-4">
              <li class="flex items-center">
                <svg class="w-4 h-4 text-green-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
            ...
            <% else %>
              <button phx-click="upgrade_to_pro" class="w-full bg-purple-600 text-white py-2 px-4 rounded-lg text-sm hover:bg-purple-700 transition-colors">
                Upgrade to Pro
              </button>
            <% end %>
          </div>
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
              ...
              <div class="text-right">
                <p class="text-sm font-medium text-gray-900"><%= Streampai.Pricing.monthly_price_formatted() %></p>
