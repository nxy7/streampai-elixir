defmodule Streampai.Stream.ViewerLinkingTest do
  ...
  alias Streampai.Stream.{Viewer, ViewerIdentity, ViewerLinking, ChatMessage, StreamEvent}
  ...
  describe "link_identity/4" do
    test "creates new viewer and identity for first-time platform user", %{user: user} do
      result = ViewerLinking.link_identity(
        :twitch,
        "12345",
        "johndoe",
        user_id: user.id
    ...
    test "links to existing viewer when username similarity is high", %{user: user} do
      ...
      {:ok, existing_viewer} = Viewer.create!(%{
        display_name: "johndoe",
        user_id: user.id
      })

      {:ok, _youtube_identity} = ViewerIdentity.create!(%{
    ...
    test "returns existing identity when already linked", %{user: user} do
      ...
      {:ok, existing_identity} = ViewerIdentity.create!(%{
        viewer_id: viewer.id,
        platform: :twitch,
        platform_user_id: "12345",
        username: "testuser",
      ...
      auto_assert {:ok, %{viewer: returned_viewer, identity: returned_identity, created?: false}} <- result
    ...
    test "updates username when existing identity has different username", %{user: user} do
      ...
      auto_assert {:ok, %{viewer: returned_viewer, identity: updated_identity, created?: false}} <- result
  ...
  describe "reevaluate_user_linking/1" do
    test "performs dry run reevaluation without making changes", %{user: user} do
      ...
      {:ok, _identity} = ViewerIdentity.create!(%{
        viewer_id: viewer.id,
        platform: :twitch,
        platform_user_id: "12345",
        username: "testuser",
      ...
      result = ViewerLinking.reevaluate_user_linking(
        user_id: user.id,
        dry_run: true
      )
  ...
  describe "confidence scoring and linking methods" do
    ...
    test "respects custom confidence thresholds", %{user: user} do
      ...
      auto_assert {:ok, %{identity: identity}} <- result
      assert not is_nil(identity.confidence_score)
    end
  end
  ...
  describe "batch processing" do
    test "groups linking operations with batch IDs", %{user: user} do
      ...
      result1 = ViewerLinking.link_identity(
        :twitch,
        "user1",
        "testuser1",
        user_id: user.id,
      ...
      result2 = ViewerLinking.link_identity(
        :youtube,
        "user2",
        "testuser2",
        user_id: user.id,
