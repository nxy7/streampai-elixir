					...
					if (x) {
								...
								for (const { key: I, descriptor: R } of K(
									...
									if (D(N).includes("Function") && typeof N == "function") {
										...
										f.mockRestore = f.mockReset = () => (
											$.call(f), f.mockImplementation(L), f
										);
									}
								}
						};
						...
						if (n.type === "automock") {
							...
							P.mockRestore = P.mockReset = () => (
								I.call(P), P.mockImplementation(_), P
							);
						}
	...
	let e = Re(n),
		t = (a, o, l, d) => {
			let h = "",
				g = 0;
			do
				(h += a.substring(g, d) + l), (g = d + o.length), (d = a.indexOf(o, g));
			while (~d);
...
function ze() {
	if (te) return V;
	te = 1;
	var n, e, t, r, i, s, a, o, l, d, h, g, w, b, y, c, u, x, M;
	return (
		(w =
			/\/(?![*\/])(?:\[(?:(?![\]\\]).|\\.)*\]|(?![\/\\]).|\\.)*(\/[$_\u200C\u200D\p{ID_Continue}]*|\\)?/uy),
		(g =
		...
		(V = function* (_, { jsx: P = !1 } = {}) {
			...
			for (
				...
				if (((l.lastIndex = p), (f = l.exec(_)))) {
					(p = l.lastIndex),
						d.test(f[0]) &&
							((S = !1), a.test(m) && (m = "?NoLineTerminatorHere")),
						yield {
