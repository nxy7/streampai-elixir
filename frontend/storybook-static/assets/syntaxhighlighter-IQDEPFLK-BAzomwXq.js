import {
	...
	a as bt,
	S as xt,
} from "./DocsRenderer-GHJI37HO-sAbIdlGz.js";
import { a2 as Z, a3 as F, a4 as vt } from "./iframe-BQDcX1su.js";
...
var At = F({
		...
		"../node_modules/refractor/lang/css.js"(e, r) {
			...
			function t(a) {
				(function (n) {
					...
					u &&
						(u.tag.addInlined("style", "css"),
						u.tag.addAttribute("style", "css"));
				})(a);
			}
		},
	}),
			...
			function t(a) {
				(a.languages.graphql = {
					...
					a.hooks.add("after-tokenize", function (n) {
						...
						function f(A, i) {
							for (var s = 1, p = u; p < o.length; p++) {
								...
								if (g.type === "punctuation" && typeof m == "string") {
									if (A.test(m)) s++;
									else if (i.test(m) && (s--, s === 0)) return p;
								}
							}
							return -1;
						}
						function h(A, i) {
							var s = A.alias;
							s ? Array.isArray(s) || (A.alias = s = [s]) : (A.alias = s = []),
								s.push(i);
						}
						for (; u < o.length; ) {
							var C = o[u++];
							if (C.type === "keyword" && C.content === "mutation") {
								...
								if (
									...
									for (; u < b; u++) {
										var y = d(0);
										y.type === "variable" &&
											(h(y, "variable-input"), x.push(y.content));
									}
									u = b + 1;
								}
								if (
									...
									d(0).content === "{" &&
									(u++, h(d(0), "property-mutation"), x.length > 0)
								) {
									var S = f(/^\{$/, /^\}$/);
									if (S === -1) continue;
									for (var E = u; E < S; E++) {
			...
			function t(a) {
				(function (n) {
					...
					for (
						...
						var l = u[d],
							f = n.languages.javascript[l];
						n.util.type(f) === "RegExp" &&
							(f = n.languages.javascript[l] = { pattern: f });
						var h = f.inside || {};
	...
	Qe = F({
		"../node_modules/refractor/lang/jsx.js"(e, r) {
			...
			function t(a) {
				(function (n) {
					var o = n.util.clone(n.languages.javascript),
						u = /(?:\s|\/\/.*(?!.)|\/\*(?:[^*]|\*(?!\/))\*\/)/.source,
						d = /(?:\{(?:\{(?:\{[^{}]*\}|[^{}])*\}|[^{}])*\})/.source,
						l = /(?:\{<S>*\.{3}(?:[^{}]|<BRACES>)*\})/.source;
					function f(x, b) {
					...
					(l = f(l).source),
						(n.languages.jsx = n.languages.extend("markup", o)),
						(n.languages.jsx.tag.pattern = f(
						...
						(n.languages.jsx.tag.inside.tag.inside["class-name"] =
							/^[A-Z]\w*(?:\.[A-Z]\w*)*$/),
						(n.languages.jsx.tag.inside.comment = o.comment),
							...
							for (var b = [], y = 0; y < x.length; y++) {
								...
								if (
									(typeof S != "string" &&
										(S.type === "tag" &&
										S.content[0] &&
										S.content[0].type === "tag"
											? S.content[0].content[0].content === "</"
												? b.length > 0 &&
													b[b.length - 1].tagName ===
														h(S.content[0].content[1]) &&
													b.pop()
									...
									y < x.length - 1 &&
										(typeof x[y + 1] == "string" ||
											x[y + 1].type === "plain-text") &&
										((_ += h(x[y + 1])), x.splice(y + 1, 1)),
										y > 0 &&
											(typeof x[y - 1] == "string" ||
												x[y - 1].type === "plain-text") &&
											((_ = h(x[y - 1]) + _), x.splice(y - 1, 1), y--),
										(x[y] = new n.Token("plain-text", _, null, _));
								}
				...
				(function (n) {
					...
					(n.languages.markdown = n.languages.extend("markup", {})),
						n.languages.insertBefore("markdown", "prolog", {
							...
							table: {
								pattern: RegExp("^" + l + f + "(?:" + l + ")*", "m"),
								inside: {
									"table-data-rows": {
										pattern: RegExp("^(" + l + f + ")(?:" + l + ")*$"),
										lookbehind: !0,
										inside: {
											"table-data": {
												pattern: RegExp(d),
						...
						["url", "bold", "italic", "strike"].forEach(function (y) {
							["url", "bold", "italic", "strike", "code-snippet"].forEach(
								function (S) {
									y !== S &&
										(n.languages.markdown[y].inside.content.inside[S] =
											n.languages.markdown[S]);
								},
							);
						}),
						n.hooks.add("after-tokenize", function (y) {
							if (y.language !== "markdown" && y.language !== "md") return;
							function S(E) {
								if (!(!E || typeof E == "string"))
									for (var _ = 0, A = E.length; _ < A; _++) {
										var i = E[_];
										if (i.type !== "code") {
											S(i.content);
											continue;
						...
						n.hooks.add("wrap", function (y) {
							if (y.type === "code-block") {
								for (var S = "", E = 0, _ = y.classes.length; E < _; E++) {
									var A = y.classes[E],
										i = /language-(.+)/.exec(A);
									if (i) {
										S = i[1];
								...
								var s = n.languages[S];
								if (s) y.content = n.highlight(b(y.content.value), s, S);
									...
									(y.attributes.id = p),
										n.plugins.autoloader.loadLanguages(S, function () {
											var g = document.getElementById(p);
											g &&
												(g.innerHTML = n.highlight(
													g.textContent,
													n.languages[S],
													S,
												));
					...
					var h = RegExp(n.languages.markup.tag.pattern.source, "gi"),
						C = { amp: "&", lt: "<", gt: ">", quot: '"' },
						x = String.fromCodePoint || String.fromCharCode;
					function b(y) {
						var S = y.replace(h, "");
						return (
							(S = S.replace(/&(\w{1,8}|#x?[\da-f]{1,8});/gi, function (E, _) {
								if (((_ = _.toLowerCase()), _[0] === "#")) {
									var A;
									return (
										_[1] === "x"
											? (A = parseInt(_.slice(2), 16))
											: (A = Number(_.slice(1))),
										x(A)
									);
								} else {
		...
		"../node_modules/refractor/lang/typescript.js"(e, r) {
			...
			function t(a) {
				(function (n) {
					...
					delete o["class-name"],
						(n.languages.typescript["class-name"].inside = o),
			...
			function n(o) {
				o.register(t),
					...
					(function (u) {
						...
						(u.languages.tsx = u.languages.extend("jsx", d)),
							delete u.languages.tsx.parameter,
							delete u.languages.tsx["literal-property"];
			...
			function t(a) {
				(function (n) {
					...
					(n.languages.yaml = {
						...
						string: { pattern: h(f), lookbehind: !0, greedy: !0 },
						number: {
							pattern: h(
		...
		"../node_modules/xtend/immutable.js"(e, r) {
			...
			function a() {
				for (var n = {}, o = 0; o < arguments.length; o++) {
					var u = arguments[o];
					for (var d in u) t.call(u, d) && (n[d] = u[d]);
				}
				return n;
			}
		},
	...
	Lt = F({
		"../node_modules/hastscript/node_modules/property-information/lib/util/merge.js"(
			...
			function n(o) {
				...
				return new a(t.apply(null, d), t.apply(null, l), C);
			}
		},
	}),
	De = F({
	...
	Re = F({
		"../node_modules/hastscript/node_modules/property-information/lib/util/types.js"(
			...
			(e.boolean = t()),
				(e.booleanish = t()),
				(e.overloadedBoolean = t()),
				(e.number = t()),
				(e.spaceSeparated = t()),
				(e.commaSeparated = t()),
				(e.commaOrSpaceSeparated = t());
			function t() {
				return Math.pow(2, ++r);
			}
		},
		...
		"../node_modules/hastscript/node_modules/property-information/lib/util/create.js"(
			...
			function o(u) {
				var d = u.space,
					...
					S;
				for (y in h)
					(S = new n(y, C(f, y), h[y], d)),
						l.indexOf(y) !== -1 && (S.mustUseProperty = !0),
						(x[y] = S),
						(b[t(y)] = y),
						(b[t(S.attribute)] = y);
				return new a(x, b, d);
		...
		"../node_modules/hastscript/node_modules/property-information/find.js"(
			...
			function f(y, S) {
				var E = t(S),
					_ = S,
					A = n;
				return E in y.normal
					? y.property[y.normal[E]]
					: (E.length > 4 &&
							E.slice(0, 4) === o &&
							u.test(S) &&
							(S.charAt(4) === "-" ? (_ = h(S)) : (S = C(S)), (A = a)),
						new A(_, S));
			}
			function h(y) {
	...
	zt = F({
		"../node_modules/hast-util-parse-selector/index.js"(e, r) {
			...
			function a(n, o) {
				for (
					var u = n || "", d = o || "div", l = {}, f = 0, h, C, x;
					f < u.length;
				)
					(t.lastIndex = f),
						(x = t.exec(u)),
						(h = u.slice(f, x ? x.index : u.length)),
						h &&
							(C
								? C === "#"
									? (l.id = h)
	...
	Ut = F({
		"../node_modules/hastscript/node_modules/comma-separated-tokens/index.js"(
			...
			function n(u) {
				for (
					var d = [], l = String(u || a), f = l.indexOf(r), h = 0, C = !1, x;
					!C;
				)
					f === -1 && ((f = l.length), (C = !0)),
						(x = l.slice(h, f).trim()),
			...
			function o(u, d) {
				var l = d || {},
					...
					h = l.padRight ? t : a;
				return (
					u[u.length - 1] === a && (u = u.concat(a)), u.join(h + r + f).trim()
				);
			}
		},
	}),
		...
		"../node_modules/hastscript/factory.js"(e, r) {
			...
			function l(E, _, A) {
				...
				function s(g, m) {
					var c = n(g, _),
						...
						v = c.tagName.toLowerCase(),
						k;
					if (
						((c.tagName = i && d.call(i, v) ? i[v] : v),
						m && f(m, c) && (w.unshift(m), (m = null)),
						m)
					)
						for (k in m) p(c.properties, k, m[k]);
					return (
						C(c.children, w),
						c.tagName === "template" &&
					...
					c == null ||
						...
						typeof k == "string" &&
							(w.spaceSeparated
								...
								: w.commaSeparated
									? (k = u(k))
									: w.commaOrSpaceSeparated && (k = o(u(k).join(" ")))),
						v === "style" && typeof c != "string" && (k = y(k)),
						v === "className" && g.className && (k = g.className.concat(k)),
						(g[v] = x(w, v, k)));
				}
			}
			function f(E, _) {
				return typeof E == "string" || "length" in E || h(_.tagName, E);
			}
			function h(E, _) {
				var A = _.type;
				return E === "input" || !A || typeof A != "string"
					? !1
					: typeof _.children == "object" && "length" in _.children
						? !0
						: ((A = A.toLowerCase()),
							E === "button"
								? A !== "menu" &&
			...
			function C(E, _) {
				...
				if (typeof _ == "string" || typeof _ == "number") {
					E.push({ type: "text", value: String(_) });
					return;
				}
				if (typeof _ == "object" && "length" in _) {
					for (A = -1, i = _.length; ++A < i; ) C(E, _[A]);
					return;
				}
				if (typeof _ != "object" || !("type" in _))
					throw new Error("Expected node, nodes, or string, got `" + _ + "`");
				E.push(_);
			}
			function x(E, _, A) {
				var i, s, p;
				if (typeof A != "object" || !("length" in A)) return b(E, _, A);
				for (s = A.length, i = -1, p = []; ++i < s; ) p[i] = b(E, _, A[i]);
				return p;
			}
			function b(E, _, A) {
				var i = A;
			...
			function y(E) {
				var _ = [],
					A;
				for (A in E) _.push([A, E[A]].join(": "));
				return _.join("; ");
			}
			function S(E) {
				for (var _ = E.length, A = -1, i = {}, s; ++A < _; )
		...
		"../node_modules/refractor/node_modules/parse-entities/decode-entity.browser.js"(
			...
			function n(o) {
				var u = "&" + o + ";",
					d;
				return (
					(t = t || document.createElement("i")),
					(t.innerHTML = u),
					(d = t.textContent),
					(d.charCodeAt(d.length - 1) === a && o !== "semi") || d === u ? !1 : d
				);
			}
		},
	}),
	Qt = F({
		"../node_modules/refractor/node_modules/parse-entities/index.js"(e, r) {
			...
			var O = 1,
				...
				L = 7,
				I = {};
			(I[O] = "Named character references must be terminated by a semicolon"),
				(I[H] =
					"Numeric character references must be terminated by a semicolon"),
				(I[z] = "Named character references cannot be empty"),
				(I[X] = "Numeric character references cannot be empty"),
				(I[J] = "Named character references must be known"),
				(I[W] = "Numeric character references cannot be disallowed"),
				(I[L] =
			...
			function B(j, $) {
				var T = {},
					...
					U;
				$ || ($ = {});
				for (U in C) (K = $[U]), (T[U] = K ?? C[U]);
				return (
					(T.position.indent || T.position.start) &&
						((T.indent = T.position.indent || []),
						(T.position = T.position.start)),
					Q(j, T)
				);
			}
			function Q(j, $) {
				...
				for (
					...
					if (
						(ne === b && (oe = gt[He] || 1), (ne = j.charCodeAt(te)), ne === E)
					) {
						if (
							((D = j.charCodeAt(te + 1)),
							D === x ||
						...
						for (
							ue = te + 1,
								...
								Pe = N[ee],
								q--;
							++q < Fe && ((D = j.charCodeAt(q)), !!Pe(D));
						)
							(P += f(D)), ee === c && l.call(t, P) && ((ge = P), (se = t[P]));
						(ie = j.charCodeAt(q) === _),
							ie &&
								(q++, (Ne = ee === c ? d(P) : !1), Ne && ((ge = P), (se = Ne))),
							(V = 1 + q - ue),
							(!ie && !K) ||
								(P
									? ee === c
							...
							R
								? (Ge(),
									...
									_e.offset++,
									re &&
										re.call(ke, R, { start: ye, end: _e }, j.slice(ue - 1, q)),
									(ye = _e))
								: ((P = j.slice(ue - 1, q)),
									(le += P),
									(oe += P.length),
				...
				return je.join("");
				function be() {
					return { line: Ue, column: oe, offset: te + (Ce.offset || 0) };
				}
				function ft(Ze, We) {
					var Le = be();
					(Le.column += We), (Le.offset += We), pe.call(me, I[Ze], Le, Ze);
				}
				function Ge() {
					le &&
						(je.push(le),
						U && U.call(de, le, { start: ye, end: be() }),
						(le = ""));
				}
			}
			function M(j) {
			...
			var t =
				...
				a = (function (n) {
					var o = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i,
						...
						l = {
							...
							util: {
								encode: function i(s) {
									return s instanceof f
										? new f(s.type, i(s.content), s.alias)
										: Array.isArray(s)
											? s.map(i)
											: s
													.replace(/&/g, "&amp;")
								...
								objId: function (i) {
									return (
										i.__id || Object.defineProperty(i, "__id", { value: ++u }),
										i.__id
									);
								},
								clone: function i(s, p) {
									...
									switch (l.util.type(s)) {
										case "Object":
											if (((m = l.util.objId(s)), p[m])) return p[m];
											(g = {}), (p[m] = g);
											for (var c in s)
								...
								setLanguage: function (i, s) {
									(i.className = i.className.replace(RegExp(o, "gi"), "")),
										i.classList.add("language-" + s);
								},
								currentScript: function () {
									...
									try {
										...
										if (i) {
											...
											for (var p in s) if (s[p].src == i) return s[p];
										}
										return null;
									}
								},
								...
								insertBefore: function (i, s, p, g) {
									...
									for (var w in m)
										if (m.hasOwnProperty(w)) {
											if (w == s)
												for (var v in p) p.hasOwnProperty(v) && (c[v] = p[v]);
											p.hasOwnProperty(w) || (c[w] = m[w]);
										}
									var k = g[i];
									return (
										(g[i] = c),
										l.languages.DFS(l.languages, function (N, O) {
											O === k && N != i && (this[N] = c);
										}),
										c
									);
							...
							highlightAllUnder: function (i, s, p) {
								...
								l.hooks.run("before-highlightall", g),
									(g.elements = Array.prototype.slice.apply(
										g.container.querySelectorAll(g.selector),
									)),
									l.hooks.run("before-all-elements-highlight", g);
								for (var m = 0, c; (c = g.elements[m++]); )
									l.highlightElement(c, s === !0, g.callback);
							},
							highlightElement: function (i, s, p) {
								var g = l.util.getLanguage(i),
									m = l.languages[g];
								l.util.setLanguage(i, g);
								var c = i.parentElement;
								c &&
									c.nodeName.toLowerCase() === "pre" &&
									l.util.setLanguage(c, g);
								var w = i.textContent,
									v = { element: i, language: g, grammar: m, code: w };
								function k(O) {
									(v.highlightedCode = O),
										l.hooks.run("before-insert", v),
										(v.element.innerHTML = v.highlightedCode),
										l.hooks.run("after-highlight", v),
										l.hooks.run("complete", v),
										p && p.call(v.element);
								}
								if (
									(l.hooks.run("before-sanity-check", v),
									(c = v.element.parentElement),
									c &&
										c.nodeName.toLowerCase() === "pre" &&
								...
								} else k(l.highlight(v.code, v.grammar, v.language));
							},
							highlight: function (i, s, p) {
								var g = { code: i, grammar: s, language: p };
								if ((l.hooks.run("before-tokenize", g), !g.grammar))
									throw new Error(
										'The language "' + g.language + '" has no grammar.',
									);
								return (
									(g.tokens = l.tokenize(g.code, g.grammar)),
									l.hooks.run("after-tokenize", g),
									f.stringify(l.util.encode(g.tokens), g.language)
								);
							},
					...
					f.stringify = function i(s, p) {
						...
						c &&
							(Array.isArray(c)
								? Array.prototype.push.apply(m.classes, c)
								: m.classes.push(c)),
							l.hooks.run("wrap", m);
						var w = "";
						...
						for (var w in p)
							if (!(!p.hasOwnProperty(w) || !p[w])) {
								...
								for (var k = 0; k < v.length; ++k) {
									...
									for (
										var W = N.pattern || N, L = g.next, I = m;
										L !== s.tail && !(c && I >= c.reach);
										...
										if (!(B instanceof f)) {
											var Q = 1,
												M;
											if (z) {
												if (((M = h(W, I, i, H)), !M || M.index >= i.length))
													break;
												var T = M.index,
													ce = M.index + M[0].length,
													j = I;
												for (j += L.value.length; T >= j; )
													(L = L.next), (j += L.value.length);
												if (
													((j -= L.value.length), (I = j), L.value instanceof f)
												)
													continue;
												for (
													var $ = L;
													$ !== s.tail &&
													(j < ce || typeof $.value == "string");
													$ = $.next
												)
													Q++, (j += $.value.length);
												Q--, (B = i.slice(I, j)), (M.index -= I);
											} else if (((M = h(W, 0, B, H)), !M)) continue;
											var T = M.index,
												K = M[0],
												U = B.slice(0, T),
												re = B.slice(T + K.length),
												pe = I + B.length;
											c && pe > c.reach && (c.reach = pe);
											var de = L.prev;
											U && ((de = b(s, de, U)), (I += U.length)), y(s, de, Q);
											var ke = new f(w, O ? l.tokenize(K, O) : K, X, K);
											if (((L = b(s, de, ke)), re && b(s, L, re), Q > 1)) {
												var me = { cause: w + "," + k, reach: pe };
												C(i, s, p, L.prev, I, me),
													c && me.reach > c.reach && (c.reach = me.reach);
											}
										}
									}
								}
					...
					function x() {
						var i = { value: null, prev: null, next: null },
							s = { value: null, prev: i, next: null };
						(i.next = s), (this.head = i), (this.tail = s), (this.length = 0);
					}
					function b(i, s, p) {
						var g = s.next,
							m = { value: p, prev: s, next: g };
						return (s.next = m), (g.prev = m), i.length++, m;
					}
					function y(i, s, p) {
						for (var g = s.next, m = 0; m < p && g !== i.tail; m++) g = g.next;
									...
									n.addEventListener(
										...
										function (i) {
											var s = JSON.parse(i.data),
												...
												g = s.code,
												m = s.immediateClose;
											n.postMessage(l.highlight(g, l.languages[p], p)),
												m && n.close();
										},
										!1,
									)),
					...
					E &&
						((l.filename = E.src),
						E.hasAttribute("data-manual") && (l.manual = !0));
					function _() {
						l.manual || l.highlightAll();
					}
					if (!l.manual) {
						var A = document.readyState;
						A === "loading" || (A === "interactive" && E && E.defer)
	...
	an = F({
		"../node_modules/refractor/core.js"(e, r) {
			...
			function S(c, w) {
				...
				for (N in k)
					for (
						O = k[N], O = typeof O == "string" ? [O] : O, H = O.length, z = -1;
						++z < H;
					)
						v[O[z]] = v[N];
			}
			function E(c, w) {
				...
				if (b.util.type(w) === "Object") (k = w), (w = null);
				else {
					if (typeof w != "string")
						throw new Error("Expected `string` for `name`, got `" + w + "`");
					if (C.call(b.languages, w)) k = b.languages[w];
					else
						throw new Error("Unknown language: `" + w + "` is not registered");
				}
				return v.call(this, c, k, w);
			}
			function _(c) {
				if (typeof c != "string")
			...
			function A() {
				var c = b.languages,
					w = [],
					v;
				for (v in c) C.call(c, v) && typeof c[v] == "object" && w.push(v);
				return w;
			}
			function i(c, w, v) {
				var k;
				return typeof c == "string"
					...
					: b.util.type(c) === "Array"
						...
						: ((k = {
								type: c.type,
								content: b.Token.stringify(c.content, w, v),
								tag: "span",
							...
							c.alias && (k.classes = k.classes.concat(c.alias)),
							b.hooks.run("wrap", k),
							n(k.tag + "." + k.classes.join("."), g(k.attributes), k.content));
			}
			function s(c, w) {
				for (var v = [], k = c.length, N = -1, O; ++N < k; )
					(O = c[N]), O !== "" && O !== null && O !== void 0 && v.push(O);
				for (N = -1, k = v.length; ++N < k; )
					(O = v[N]), (v[N] = b.Token.stringify(O, w, v));
				return v;
			}
			function p(c) {
				return c;
			...
			function m() {
				...
				return v;
				function v() {
					c ? (t.Prism = w) : delete t.Prism, (c = void 0), (w = void 0);
				}
			}
		},
	}),
	rn = Z(vt(), 1);
const { logger: on } = __STORYBOOK_MODULE_CLIENT_LOGGER__,
	{ global: ln } = __STORYBOOK_MODULE_GLOBAL__;
function Ke(e, r) {
	var t = Object.keys(e);
	if (Object.getOwnPropertySymbols) {
...
function fe(e) {
	for (var r = 1; r < arguments.length; r++) {
		...
		r % 2
			? Ke(Object(t), !0).forEach(function (a) {
					Ve(e, a, t[a]);
				})
			: Object.getOwnPropertyDescriptors
				? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
				: Ke(Object(t)).forEach(function (a) {
...
function un(e) {
	...
	var r = e.join(".");
	return Ie[r] || (Ie[r] = sn(e)), Ie[r];
}
function cn(e) {
	var r = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {},
		t = arguments.length > 2 ? arguments[2] : void 0,
		a = e.filter(function (o) {
			return o !== "token";
		}),
		n = un(a);
...
function qe(e) {
	...
	if (f) {
		var C = pn(t, o),
			x;
		if (!o) x = fe(fe({}, d), {}, { className: Xe(d.className) });
		else {
			var b = Object.keys(t).reduce(function (_, A) {
					return (
			...
			x = fe(
				...
				{},
				{
					...
					style: cn(d.className, Object.assign({}, d.style, n), t),
				},
			);
		}
		var E = C(r.children);
		return G.createElement(f, bt({ key: u }, x), E);
	}
}
var dn = Z(At()),
	gn = dn.default,
...
function ae(e) {
	for (var r = 1; r < arguments.length; r++) {
		var t = arguments[r] != null ? arguments[r] : {};
		r % 2
			? Ye(Object(t), !0).forEach(function (a) {
					Ve(e, a, t[a]);
				})
			: Object.getOwnPropertyDescriptors
				? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
				: Ye(Object(t)).forEach(function (a) {
						Object.defineProperty(e, a, Object.getOwnPropertyDescriptor(t, a));
					});
	}
	return e;
...
function Un(e) {
	var r = e.codeString,
		t = e.codeStyle,
		a = e.containerStyle,
		n = a === void 0 ? { float: "left", paddingRight: "10px" } : a,
		o = e.numberStyle,
		u = o === void 0 ? {} : o,
		d = e.startingLineNumber;
	return G.createElement(
		"code",
...
function Ae(e) {
	var r = e.children,
		...
		x = e.wrapLines,
		b = x === void 0 ? !1 : x,
		y = b ? ae({}, typeof d == "function" ? d(t) : d) : {};
	if (
		((y.className = y.className
			? [].concat(Te(y.className.trim().split(/\s+/)), Te(f))
			: f),
		t && o)
	) {
		var S = st(a, t, n);
	...
	return (
		C & h && (y.style = ae({ display: "flex" }, y.style)),
		{ type: "element", tagName: "span", properties: y, children: r }
	);
}
function ut(e) {
	for (
		...
		var n = e[a];
		if (n.type === "text")
			t.push(Ae({ children: [n], className: Te(new Set(r)) }));
		else if (n.children) {
...
`,
					...
					if (v === 0) {
						var O = h
								.slice(x + 1, b)
								.concat(
									Ae({ children: [N], className: p.properties.className }),
								),
							H = E(O, k);
						C.push(H);
					} else if (v === c.length - 1) {
						var z = h[b + 1] && h[b + 1].children && h[b + 1].children[0],
							X = { type: "text", value: "".concat(w) };
						if (z) {
							var J = Ae({ children: [X], className: p.properties.className });
							h.splice(b + 1, 0, J);
						} else {
							var W = [X],
								L = E(W, k, p.properties.className);
							C.push(L);
						}
					} else {
						var I = [N],
							B = E(I, k, p.properties.className);
						C.push(B);
					}
				}),
					(x = b);
...
function Wn(e) {
	var r = e.astGenerator,
		t = e.language,
		a = e.code,
		n = e.defaultCodeValue;
	if (ct(r)) {
		var o = qn(r, t);
		return t === "text"
			? { value: n, language: "text" }
			: o
				? r.highlight(t, a)
				: r.highlightAuto(a);
	}
	try {
		return t && t !== "text" ? { value: r.highlight(a, t) } : { value: n };
	} catch {
		return { value: n };
	}
}
function Kn(e, r) {
	return function (t) {
		var a,
			...
			S = t.showLineNumbers,
			E = S === void 0 ? !1 : S,
			_ = t.showInlineLineNumbers,
			A = _ === void 0 ? !0 : _,
			i = t.startingLineNumber,
			s = i === void 0 ? 1 : i,
			p = t.lineNumberContainerStyle,
			g = t.lineNumberStyle,
			m = g === void 0 ? {} : g,
			c = t.wrapLines,
			w = t.wrapLongLines,
			...
			J = X === void 0 ? "code" : X,
			W = t.code,
			L = W === void 0 ? (Array.isArray(u) ? u[0] : u) || "" : W,
			I = t.astGenerator,
			B = wt(t, Bn);
		I = I || e;
		var Q = E
				...
				: null,
			M = l.hljs || l['pre[class*="language-"]'] || { backgroundColor: "#fff" },
			ce = ct(I) ? "hljs" : "prismjs",
			j = y
				? Object.assign({}, B, { style: Object.assign({}, M, h) })
				: Object.assign({}, B, {
						className: B.className
							? "".concat(ce, " ").concat(B.className)
							: ce,
						style: Object.assign({}, h),
					});
		if (
			(v
				? (x.style = ae({ whiteSpace: "pre-wrap" }, x.style))
				: (x.style = ae({ whiteSpace: "pre" }, x.style)),
			!I)
		)
			return G.createElement(z, j, Q, G.createElement(J, x, L));
		((c === void 0 && O) || v) && (c = !0), (O = O || Zn);
		var $ = [{ type: "text", value: L }],
			T = Wn({ astGenerator: I, language: o, code: L, defaultCodeValue: $ });
		T.language === null && (T.value = $);
		var K =
				(a =
					(n = L.match(/\n/g)) === null || n === void 0 ? void 0 : n.length) !==
					null && a !== void 0
					? a
					: 0,
			U = s + K,
			re = Gn(T, c, N, E, A, s, U, m, v);
		return G.createElement(
			z,
			j,
			G.createElement(
				...
				x,
				!A && Q,
				O({ rows: re, stylesheet: l, useInlineStyles: y }),
			),
		);
	};
}
var Be = Z(an()),
	Me = Kn(Be.default, {});
Me.registerLanguage = function (e, r) {
	return Be.default.register(r);
};
Me.alias = function (e, r) {
	return Be.default.alias(e, r);
};
var ze = Me,
	{ window: Xn } = ln,
	Yn = {
...
Object.entries(Yn).forEach(([e, r]) => {
	ze.registerLanguage(e, r);
});
var Vn = (0, rn.default)(2)((e) =>
	...
	ea = ({ children: e, className: r }) =>
		G.createElement(xt, { horizontal: !0, vertical: !0, className: r }, e),
	ta = Ee(ea)({ position: "relative" }, ({ theme: e }) => Vn(e)),
	na = Ee.pre(({ theme: e, padded: r }) => ({
		display: "flex",
	...
	ra = ({ rows: e, stylesheet: r, useInlineStyles: t }) =>
		e.map((a, n) =>
			qe({
				node: pt(a),
				stylesheet: r,
	...
	oa = (e, r) =>
		r
			? e
				? ({ rows: t, ...a }) => e({ rows: t.map((n) => pt(n)), ...a })
				: ra
			: e,
	dt = ({
		children: e,
		...
		let [x, b] = we.useState(!1),
			y = we.useCallback(
				(E) => {
					E.preventDefault(),
						Jn(h)
							.then(() => {
								b(!0), Xn.setTimeout(() => b(!1), 1500);
							})
							.catch(on.error);
				},
				[h],
		...
		return G.createElement(
			Qn,
			{ bordered: a, padded: n, showLineNumbers: l, className: d },
			G.createElement(
				ta,
				null,
				G.createElement(
			...
			t
				? G.createElement(yt, {
						actionItems: [{ title: x ? "Copied" : "Copy", onClick: y }],
					})
				: null,
		);
	};
dt.registerLanguage = (...e) => ze.registerLanguage(...e);
var ca = dt;
export { dt as SyntaxHighlighter, ca as default, Yn as supportedLanguages };
