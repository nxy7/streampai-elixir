...
type StreamStatus = "offline" | "ready" | "starting" | "live" | "stopping";
...
function StreamContent() {
	...
	const chatWidgetConfig = useWidgetConfig(
		() => user().id,
		() => "chat_widget" as const,
	);
	const chatNameSaturation = () =>
		(chatWidgetConfig.data()?.config as Record<string, unknown> | undefined)
			?.name_saturation as number | undefined;
	const chatNameLightness = () =>
		(chatWidgetConfig.data()?.config as Record<string, unknown> | undefined)
			?.name_lightness as number | undefined;
	...
	const streamStatus = (): StreamStatus => {
		...
		if (actorStatus === "ready") return "ready";
		...
		try {
			const result = await goLive({
				input: {
					...
					platforms: streamMetadata.enabledPlatforms?.length
						? streamMetadata.enabledPlatforms
						: undefined,
					metadata: {
					...
					<div class="mb-4 flex items-center justify-between px-6 pt-6">
						<div class="flex items-center gap-4">
							...
							<Show
								...
								<Badge
									variant={streamStatus() === "ready" ? "success" : "neutral"}>
					...
					<Show
						...
						<div
							...
							<LiveStreamControlCenter
								...
								isStopping={streamStatus() === "stopping"}
								nameLightness={chatNameLightness()}
								nameSaturation={chatNameSaturation()}
						...
						<div class="px-6 pb-6">
							...
							<div class="mb-6">
								<LiveInputPreview
									...
									previewHlsUrl={streamActor.previewHlsUrl()}
								/>
							</div>
							...
							<div class="flex items-center space-x-3">
								<Show
									...
									when={
										streamStatus() === "offline" || streamStatus() === "ready"
									}>
									<Button
										disabled={
