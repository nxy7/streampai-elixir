...
function parseBackendConfig(
	...
	return {
		...
		entryMethodText:
			backendConfig.entry_method_text ?? DEFAULT_CONFIG.entryMethodText,
		showEntryMethod:
			backendConfig.show_entry_method ?? DEFAULT_CONFIG.showEntryMethod,
		showProgressBar:
			backendConfig.show_progress_bar ?? DEFAULT_CONFIG.showProgressBar,
		targetParticipants:
			backendConfig.target_participants ?? DEFAULT_CONFIG.targetParticipants,
		patreonMultiplier:
			backendConfig.patreon_multiplier ?? DEFAULT_CONFIG.patreonMultiplier,
		patreonBadgeText:
			backendConfig.patreon_badge_text ?? DEFAULT_CONFIG.patreonBadgeText,
		winnerAnimation:
			(backendConfig.winner_animation as GiveawayConfig["winnerAnimation"]) ??
			DEFAULT_CONFIG.winnerAnimation,
		...
		fontSize:
			(backendConfig.font_size as GiveawayConfig["fontSize"]) ??
			DEFAULT_CONFIG.fontSize,
		showPatreonInfo:
			backendConfig.show_patreon_info ?? DEFAULT_CONFIG.showPatreonInfo,
	};
}
...
export default function GiveawayWidgetSettings() {
	...
	function updateConfig<K extends keyof GiveawayConfig>(
		field: K,
		value: GiveawayConfig[K],
	) {
		...
		<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
			<div class="mb-8">
				...
				<p class={`${text.body} mt-2`}>
					Configure your giveaway widget for viewer engagement.
				</p>
			</div>
								...
								<div class="space-y-3">
									...
									<label class="flex items-center">
										<input
											...
											onChange={(e) =>
												updateConfig("showDescription", e.target.checked)
											}
											class="mr-2"
										/>
									...
									<label class="flex items-center">
										<input
											...
											onChange={(e) =>
												updateConfig("showEntryMethod", e.target.checked)
											}
											class="mr-2"
										/>
									...
									<label class="flex items-center">
										<input
											...
											onChange={(e) =>
												updateConfig("showProgressBar", e.target.checked)
											}
											class="mr-2"
										/>
