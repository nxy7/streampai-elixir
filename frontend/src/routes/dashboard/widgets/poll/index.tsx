...
function parseBackendConfig(backendConfig: BackendPollConfig): PollConfig {
	return {
		...
		showPercentages:
			backendConfig.show_percentages ?? DEFAULT_CONFIG.showPercentages,
		showVoteCounts:
			backendConfig.show_vote_counts ?? DEFAULT_CONFIG.showVoteCounts,
		fontSize:
			(backendConfig.font_size as PollConfig["fontSize"]) ??
			DEFAULT_CONFIG.fontSize,
		primaryColor: backendConfig.primary_color ?? DEFAULT_CONFIG.primaryColor,
		secondaryColor:
			backendConfig.secondary_color ?? DEFAULT_CONFIG.secondaryColor,
		backgroundColor:
		...
		animationType:
			(backendConfig.animation_type as PollConfig["animationType"]) ??
			DEFAULT_CONFIG.animationType,
		highlightWinner:
			backendConfig.highlight_winner ?? DEFAULT_CONFIG.highlightWinner,
		autoHideAfterEnd:
			backendConfig.auto_hide_after_end ?? DEFAULT_CONFIG.autoHideAfterEnd,
...
export default function PollWidgetSettings() {
	...
	function updateConfig<K extends keyof PollConfig>(
		field: K,
		value: PollConfig[K],
	) {
								...
								<div class="space-y-3">
									...
									<label class="flex items-center">
										<input
											...
											onChange={(e) =>
												updateConfig("showVoteCounts", e.target.checked)
											}
											class="mr-2"
										/>
									...
									<label class="flex items-center">
										<input
											...
											onChange={(e) =>
												updateConfig("highlightWinner", e.target.checked)
											}
											class="mr-2"
										/>
								...
								<div class="space-y-3">
									...
									<label class="block">
										...
										<input
											...
											onInput={(e) =>
												updateConfig("winnerColor", e.target.value)
											}
										/>
									</label>
						...
						<div class={card.default}>
							...
							<div class="rounded-lg bg-gray-900 p-8">
								<PollWidget
									config={config()}
									pollStatus={
										demoMode() === "active" ? DEMO_POLL_ACTIVE : DEMO_POLL_ENDED
									}
								/>
							</div>
						</div>
