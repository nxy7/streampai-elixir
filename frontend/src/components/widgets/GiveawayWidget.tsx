...
export default function GiveawayWidget(props: GiveawayWidgetProps) {
  const participantCount = createMemo(() => {
    ...
    return props.event.type === 'update' ? props.event.participants : props.event.totalParticipants;
  });

  const patreonCount = createMemo(() => {
    if (!props.event) return 0;
    return props.event.type === 'update' ? props.event.patreons : props.event.patreonParticipants;
  });
  ...
  const progressPercentage = createMemo(() => {
    if (!props.config.showProgressBar || props.config.targetParticipants <= 0) return 0;
    return Math.min(100, (participantCount() / props.config.targetParticipants) * 100);
  });
  ...
  return (
    <div class={`giveaway-widget ${fontSizeClass()} ${winner() ? 'has-winner' : ''} ${isActive() ? 'is-active' : ''}`} style={{
      ...
      <div class="giveaway-container" style={{
        ...
        <Show when={props.config.showTitle && props.config.title}>
          <div class="giveaway-title" style={{
            "font-size": "1.5em",
            "font-weight": "bold",
        ...
        <Show when={props.config.showDescription && props.config.description}>
          <div class="giveaway-description" style={{
            "font-size": "0.9em",
            opacity: "0.9",
            "margin-bottom": "1rem",
            "line-height": "1.4"
        ...
        <div class="giveaway-status" style={{ "margin-bottom": "1rem" }}>
          <Show when={isActive()}>
            <div class="status-active" style={{
              ...
              <div class="participant-count" style={{ margin: "0.75rem 0" }}>
                ...
                <div class="count-label" style={{ "font-size": "0.875em", opacity: "0.9" }}>
                  {participantCount() === 1 ? 'Participant' : 'Participants'}
                </div>
              </div>

              <Show when={props.config.patreonMultiplier > 1 && patreonCount() > 0}>
                <div class="patreon-info" style={{
                  "margin-top": "0.5rem",
                  "font-size": "0.8em",
                  opacity: "0.9",
                  ...
                  {patreonCount()} Patreons ({props.config.patreonMultiplier}x entries)
                </div>
              </Show>

              <Show when={props.config.showEntryMethod}>
                <div class="entry-method" style={{
                  "margin-top": "0.75rem",
                  "font-size": "0.875em",
                  "font-weight": "500",
          ...
          <Show when={winner()}>
            <div class={`status-winner ${winnerAnimationClass()}`} style={{
              padding: "1rem",
              "border-radius": "0.5rem",
              background: "linear-gradient(135deg, #10b981 0%, #059669 100%)",
              color: "white"
            }}>
              <div class="winner-label" style={{
                "font-size": "1.1em",
                "font-weight": "600",
                "margin-bottom": "0.5rem",
                "text-transform": "uppercase",
              ...
              <Show when={winner()!.isPatreon}>
                <div class="winner-patreon-badge" style={{
                  display: "inline-block",
          ...
          <Show when={!isActive() && !winner()}>
            <div class="status-inactive" style={{
              ...
              <div class="inactive-label" style={{ "font-size": "1em", "font-weight": "500" }}>
        ...
        <Show when={props.config.showProgressBar && props.config.targetParticipants > 0}>
