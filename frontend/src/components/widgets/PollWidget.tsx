...
export default function PollWidget(props: PollWidgetProps) {
	const totalVotes = createMemo(() => {
		...
		return props.pollStatus.options.reduce(
	...
	const isWinning = (option: PollOption): boolean => {
		...
		const maxVotes = Math.max(
			...(props.pollStatus?.options.map((o) => o.votes) || [0]),
		);
	...
	return (
		<div
			class={`poll-widget ${widgetClasses()}`}
			style={{
				"font-family":
										...
										<div
											...
											<div
												...
												style={{
													...
													"background-color": isWinning(option)
							...
							<Show when={props.pollStatus?.endsAt}>
								{(endsAt) => (
									<div class="time-remaining">
										Ends {formatTimeRemaining(endsAt())}
									</div>
								)}
							</Show>
						</div>
