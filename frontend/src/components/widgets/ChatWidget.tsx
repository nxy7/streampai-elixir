...
export default function ChatWidget(props: ChatWidgetProps) {
  ...
  return (
    <div
      class="text-white h-full w-full flex flex-col"
      style={{
        "font-family":
          "-apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', sans-serif",
      }}
    >
      <div class="flex-1 overflow-y-hidden p-3 flex flex-col justify-end">
              ...
              <div class={`flex items-start space-x-2 ${fontClass()}`}>
                ...
                <div class="flex-1 min-w-0">
                  <Show when={props.config.showTimestamps}>
                    <span class="text-xs text-gray-500 mr-2">{formatTimestamp(message.timestamp)}</span>
                  </Show>
                  <span class="font-semibold" style={{ color: message.usernameColor }}>
                    {message.username}:
                  </span>
                  <span class="ml-1 text-gray-100">{processMessageContent(message)}</span>
                </div>
              </div>
            )}
          </For>
