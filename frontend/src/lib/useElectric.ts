...
import {
	...
	createSupportTicketsCollection,
...
export const useStreamTimers = createUserScopedHook(
	createStreamTimersCollection,
);
...
const getSupportMessagesCollection = createCollectionCache(
	createSupportMessagesCollection,
);
...
export function useTicketMessages(ticketId: () => string | undefined) {
	const query = useOptionalLiveQuery(() => {
		...
		return currentId ? getSupportMessagesCollection(currentId) : undefined;
	});
