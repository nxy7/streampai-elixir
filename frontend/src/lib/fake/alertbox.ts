...
export function generateEvent(): AlertEvent {
	const type = randomElement<AlertType>([
		"donation",
		"follow",
		"subscription",
		"raid",
	...
	switch (type) {
		case "donation": {
			...
			const message =
				Math.random() > 0.3 ? randomElement(donationMessages) : undefined;
			return {
				...baseEvent,
				amount,
				currency: "$",
		...
		case "subscription": {
			const message =
				Math.random() > 0.5 ? randomElement(subscriptionMessages) : undefined;
			return {
				...baseEvent,
				message,
		...
		case "follow": {
			const message =
				Math.random() > 0.9 ? "Thanks for the follow!" : undefined;
			return {
				...baseEvent,
				message,
