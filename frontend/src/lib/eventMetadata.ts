...
export interface EventMetadata {
	/** Emoji icon for widget displays */
	icon: string;
	label: string;
	color: string;
	bgColor: string;
	/** SVG path data for outline icon (viewBox: 0 0 24 24) */
	svgPath: string;
}
...
export function getEventSvgPath(type: string): string {
	return getEventMetadata(type).svgPath;
}
...
export function getEventGradient(type: string): string {
	return EVENT_GRADIENTS[type] || EVENT_GRADIENTS.donation;
}
