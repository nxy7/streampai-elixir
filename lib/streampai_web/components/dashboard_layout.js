...
export const DashboardSidebar = {
  mounted() {
    ...
    this.mainContent = this.el.querySelector('#main-content');

    // Initialize correct margin based on sidebar state
    this.initializeMargin = () => {
      if (window.matchMedia('(min-width: 768px)').matches) {
        const isExpanded = this.sidebar.classList.contains('w-64');
        this.mainContent.style.marginLeft = isExpanded ? '16rem' : '5rem';
      } else {
    ...
    this.toggleDesktopSidebar = () => {
      ...
      if (isExpanded) {
        ...
        // Use inline styles for more reliable margin control
        if (window.matchMedia('(min-width: 768px)').matches) {
          this.mainContent.style.marginLeft = '5rem'; // 80px = 5rem
        }
        ...
        // Use inline styles for more reliable margin control
        if (window.matchMedia('(min-width: 768px)').matches) {
          this.mainContent.style.marginLeft = '16rem'; // 256px = 16rem
        }
