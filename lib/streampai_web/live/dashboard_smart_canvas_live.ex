defmodule StreampaiWeb.DashboardSmartCanvasLive do
  @moduledoc """
  Smart Canvas page - Interactive canvas for composing widget layouts
  """
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     socket
     |> assign(page_title: "Smart Canvas", current_page: "smart-canvas")
     |> assign(widgets: widgets)
     |> assign(layout_saved: true)
     |> assign(canvas_maximized: false), layout: false}
  end
  ...
  def render(assigns) do
    ...
    <style>
      ...
      .widget-canvas-container::-webkit-scrollbar {
        display: none; /* Chrome, Safari, Opera */
      }

      /* Hide scrollbars when canvas is maximized */
      <%= if @canvas_maximized do %>
      html, body {
    ...
    >
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
          <h2 class="text-2xl font-bold text-gray-900 mb-2">Smart Canvas</h2>
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
              ...
              <p class="text-sm text-gray-600 mb-2">
                Copy this URL and add it as a Browser Source in OBS to display your Smart Canvas:
              </p>
              <div class="flex gap-2">
                <input
                  type="text"
                  readonly
                  value={"#{StreampaiWeb.Endpoint.url()}/widgets/smart-canvas/display?user_id=#{@current_user.id}"}
                  class="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white font-mono"
                  id="obs-url-input"
                />
                <button
                  phx-hook="CopyToClipboard"
                  data-clipboard-text={"#{StreampaiWeb.Endpoint.url()}/widgets/smart-canvas/display?user_id=#{@current_user.id}"}
    ...
    <!-- Canvas -->
        <div class={[
          ...
          @canvas_maximized && "!fixed !inset-0 !z-50 !m-0 !rounded-none !p-0"
        ]}>
          <div class={[
            "flex items-center justify-between mb-4 px-4 pt-4",
            @canvas_maximized && "!hidden"
          ]}>
            <div class="text-sm text-gray-400">Canvas: 1920x1080 (16:9)</div>
            <button
              phx-click="toggle_canvas_maximize"
              class="p-1.5 hover:bg-gray-800 rounded-md transition-colors text-gray-400 hover:text-white"
              title={if @canvas_maximized, do: "Exit Fullscreen", else: "Fullscreen"}
            >
              <%= if @canvas_maximized do %>
          ...
          <%= if @canvas_maximized do %>
            <button
              phx-click="toggle_canvas_maximize"
              class="fixed top-4 right-4 z-50 p-2 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors text-white shadow-lg"
          ...
          <div
            class={[
              "widget-canvas-container",
              @canvas_maximized &&
                "!fixed !inset-0 !w-screen !h-screen !flex !items-center !justify-center !m-0 !p-0"
            ]}
            id="widget-canvas"
            style="overflow: hidden;"
          >
            <div
              phx-hook="CanvasScaler"
              id="canvas-scaler"
              style="width: 1920px; height: 1080px; transform-origin: top left; margin: 0 auto;"
            >
              <div
                phx-hook="CanvasContextMenu"
                id="canvas-area"
                class="widget-canvas"
                style="width: 1920px; height: 1080px; position: relative; background: #0f0f0f; border: 2px solid #333; border-radius: 0.375rem; overflow: hidden;"
              >
                <div
                  ...
                  <%= for widget <- @widgets do %>
                    ...
                    >
                      ...
                      <div style="background: rgba(0, 0, 0, 0.2); border-radius: 0.375rem; padding: 0.5rem;">
                        ...
                        <div
                          data-connections-for={widget.id}
                          class="mt-2 pt-2 border-t border-gray-600"
                          style="font-size: 0.75rem; color: #9ca3af;"
                        >
                          <p class="font-semibold text-white mb-1">Connected to:</p>
                          <p class="connections-list text-gray-400">
                            Calculating...
                          </p>
                        </div>
                      </div>
  ...
  def handle_event("toggle_canvas_maximize", _params, socket) do
    {:noreply, assign(socket, :canvas_maximized, !socket.assigns.canvas_maximized)}
  end
end
