defmodule Streampai.LivestreamManager.Platforms.StubManager do
  @moduledoc """
  Provides a reusable stub implementation for platform managers.

  This module uses a `__using__` macro to generate all the boilerplate code
  for stub platform managers that don't have a real implementation yet.

  ## Usage

      defmodule Streampai.LivestreamManager.Platforms.TikTokManager do
        use Streampai.LivestreamManager.Platforms.StubManager, platform: :tiktok
      end
  """

  defmacro __using__(opts) do
    platform = Keyword.fetch!(opts, :platform)
    component = String.to_atom("#{platform}_manager")
    platform_display = platform |> to_string() |> String.capitalize()

    quote do
      @moduledoc """
      Manages #{unquote(platform_display)} platform integration for live streaming.
      ...
      def init({user_id, config}) do
        Logger.metadata(component: @component, user_id: user_id)
      ...
      def start_streaming(user_id, livestream_id, _opts \\ []) when is_binary(user_id) do
        case GenServer.call(via_tuple(user_id), {:start_streaming, livestream_id}) do
          :ok -> {:ok, %{platform: @platform, livestream_id: livestream_id}}
          error -> error
        end
      end
      ...
      def delete_message(user_id, message_id) when is_binary(user_id) and is_binary(message_id) do
        Logger.info("Delete message not implemented for #{unquote(platform_display)}: #{message_id}")
      ...
      def ban_user(user_id, target_user_id, reason \\ nil) when is_binary(user_id) and is_binary(target_user_id) do
        Logger.info(
          "Ban user not implemented for #{unquote(platform_display)}: #{target_user_id}, reason: #{inspect(reason)}"
        )
      ...
      def timeout_user(user_id, target_user_id, duration_seconds, reason \\ nil)
        ...
        Logger.info(
          "Timeout user not implemented for #{unquote(platform_display)}: #{target_user_id}, duration: #{duration_seconds}s, reason: #{inspect(reason)}"
        )
      ...
      def unban_user(user_id, ban_id) when is_binary(user_id) and is_binary(ban_id) do
        Logger.info("Unban user not implemented for #{unquote(platform_display)}: #{ban_id}")
        {:error, :not_implemented}
      end
      ...
      def handle_call({:start_streaming, livestream_id}, _from, state) do
        ...
        StreamEvents.emit_platform_started(state.user_id, livestream_id, @platform)
      ...
      def handle_call(:stop_streaming, _from, state) do
        ...
        if state.livestream_id do
          StreamEvents.emit_platform_stopped(state.user_id, state.livestream_id, @platform)
        end
      ...
      defp via_tuple(user_id) do
        RegistryHelpers.via_tuple(:platform_manager, user_id, @platform)
      end
    end
  end
end
