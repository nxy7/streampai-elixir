defmodule Streampai.LivestreamManager.Platforms.RumbleManager do
  @moduledoc """
  Manages Rumble platform integration for live streaming.
  ...
  def init({user_id, config}) do
    Logger.metadata(component: :rumble_manager, user_id: user_id)
  ...
  def start_streaming(user_id, livestream_id, _opts \\ []) when is_binary(user_id) do
    case GenServer.call(via_tuple(user_id), {:start_streaming, livestream_id}) do
      :ok -> {:ok, %{platform: :rumble, livestream_id: livestream_id}}
      error -> error
    end
  end
  ...
  def delete_message(user_id, message_id) when is_binary(user_id) and is_binary(message_id) do
    Logger.info("Delete message not implemented for Rumble: #{message_id}")
    {:error, :not_implemented}
  end

  @impl true
  def ban_user(user_id, target_user_id, reason \\ nil) when is_binary(user_id) and is_binary(target_user_id) do
    Logger.info("Ban user not implemented for Rumble: #{target_user_id}, reason: #{inspect(reason)}")
  ...
  def timeout_user(user_id, target_user_id, duration_seconds, reason \\ nil)
    ...
    Logger.info(
      "Timeout user not implemented for Rumble: #{target_user_id}, duration: #{duration_seconds}s, reason: #{inspect(reason)}"
    )
  ...
  def unban_user(user_id, ban_id) when is_binary(user_id) and is_binary(ban_id) do
    Logger.info("Unban user not implemented for Rumble: #{ban_id}")
    {:error, :not_implemented}
  end
  ...
  def handle_call({:start_streaming, livestream_id}, _from, state) do
    ...
    StreamEvents.emit_platform_started(state.user_id, livestream_id, :rumble)
  ...
  def handle_call(:stop_streaming, _from, state) do
    ...
    if state.livestream_id do
      StreamEvents.emit_platform_stopped(state.user_id, state.livestream_id, :rumble)
    end
