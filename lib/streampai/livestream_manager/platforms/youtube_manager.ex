defmodule Streampai.LivestreamManager.Platforms.YouTubeManager do
  ...
  def handle_info(:log_activity, state) do
    if state.is_active do
      Logger.info("[YouTubeManager:#{state.user_id}] Streaming active - #{DateTime.utc_now()}")
    else
  ...
  def terminate(_reason, _state) do
    # Cancel any scheduled activity logs to prevent messages after termination
    :ok
  end
  ...
  def handle_call({:update_stream_metadata, metadata}, _from, state) do
    Logger.info("[YouTubeManager:#{state.user_id}] Updating metadata: #{inspect(metadata)}")
    {:reply, :ok, state}
  end
